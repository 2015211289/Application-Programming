...
int cmp_bit(char s,int pos){
    uint32_t t1,t2,mask;
    unsigned char x=s;
    mask=0x000000FF;
    t1=(x|(1<<pos))&mask;
    t2=(x|(1<<(pos+1)))&mask;
    int f1=byte_cmp(t1,x);
    int f2=byte_cmp(t2,x);
    if(f1==0&&f2==0) return 4;
    else if(f1==0) return 3;
...
RadixTree insert(char *string,RadixTree T,Pattern * pattern,int pre){
    if(T==NULL){
        ...
        if(T->lf.record->len<=pos){// leaf end
            ...
            int ans2=cmp_bit(T->lf.key[pos],t);
            P->bh.ptr[ans2]=T;
            P=insert(string,P,pattern,pre);
            return P;
        }
